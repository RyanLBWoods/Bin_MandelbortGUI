 ...
 * coordinate.
 *
 * Example usage -- To obtain a 800x800 2-D array of Mandelbrot set values for
 * the initial parameter values
 *
 * MandelbrotCalculator mandelCalc = new MandelbrotCalculator(); int[][]
 * madelbrotData = mandelCalc.calcMandelbrotSet(800, 800,
...
public class MandelbrotCalculator implements Serializable {
     ...
     * + C can be applied for Z starting at the origin and a specific constant C
     * (given by its Real and cImaginary components). If the square of the
     * absolute value of Z is still inside the defined squaredRadius after
     * maxIterations then we stop iterating and return maxIterations, implicitly
     * assuming that Z will never escape the radius for the given setting of the
     * constant C. In this case, the value of C is treated as being part of the
     * Mandelbrot set, i.e. the set of starting constants C for which the value
     * of Z remains bounded within the complex plane under iteration.
     *
     * @param cReal
     *            the real component (akin to X component) of the constant C.
     * @param cImaginary
     *            the imaginary component (akin to Y component) of the constant
     *            C.
     * @param maxIterations
     ...
     *            The value used is commonly 4.0.
    ...
    public int calcMandel(double cReal, double cImaginary, int maxIterations, double radiusSquared) {
        // To work out Z_n+1 = Z_n^2 + C and establish whether C is in the
        // Mandelbrot set or not
        // we need to
        // square the current value of Z
        // add C
        // and then check whether the square of the length of Z is outside the
        // given radiusSquared
     ...
     *            (equivalent to lower bound Y value in Mandelbrot set)
     * @param maxIterations
     *            the maximum number of iterations to iterate the complex
     *            formula
     * @param radiusSquared
     *            the squared of the radius to use when determining whether Z
     *            escaped the circle in the complex plain or remained bounded.
     * @return the 2-D integer array mandelbrotData[yResolution][xResolution]
     *         containing the for each [y][x] pixel the number of iterations
